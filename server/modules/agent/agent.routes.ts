  ...
  async function cleanupProject(projectPath, sessionId = null) {
    try {
      const externalProjectsRoot = await fs.realpath(
        path.join(claudeHome(), 'external-projects')
      );
      ...
      if (sessionId) {
        try {
          const sessionPath = path.join(claudeHome(), 'sessions', sessionId);
    ...
    try {
      ...
      if (githubUrl) {
        ...
        if (projectPath) {
          ...
          targetPath = path.join(claudeHome(), 'external-projects', repoHash);
        }
